## reference: https://github.com/stephenslab/fastTopics-experiments/blob/main/code/smallsim_functions.R
source("code/misc.R")
...
loadings_scatterplot <- function (fit1, fit2, colors, xlab = "fit1",
                                  ylab = "fit2", match = FALSE) {
  if(match){
    id <- match_topics(F1 = fit1$F, F2 = fit2$F)
...
RMSE_loading <- function(fit.truth, fit, match = TRUE){
  if(match){
    id <- match_topics(F1 = fit.truth$F, F2 = fit$F)
    if(length(unique(id)) < length(id)){
      print("matching failed")
      return(Inf)
    }
  ...
  rmse <- sqrt(mean((fit.truth$L - fit$L)^2))
  return(rmse)
}
