...
```{r load-model}
model = readRDS("output/SLA/v0.4.5/sla_pmf_initLF50_K10_maxiter5000.Rds")
fit = get_multinom_from_pnmf(L = model$L, F = model$F)
```
...
p12 <- my_tsne_plot(fit,k = 1:10,num_threads = 4,verbose = FALSE)
print(p12)
```
...
p9 <- my_pca_plot(fit,pcs = 1:2,k = 1:10)
print(p9)
```
