...
## Introduction
I  applied [ebpmf_wbg](https://github.com/stephenslab/ebpmf.alpha/blob/master/R/ebpmf_wbg.R) (with $k = 10$) to [SLA](https://zihao12.github.io/ebpmf_data_analysis/data_preprocessing_sla) dataset. let's see the results.
...
```{r load-model}
model = readRDS("output/SLA/v0.4.5/sla_ebpmf_wbg_initLF50_K10_maxiter5000.Rds")
...
### t-SNE plots
```{r tsne-plot1, fig.width=8, fig.height=4.5, warning=FALSE}
p12 <- my_tsne_plot(fit,k = 1:4, n = 4000,num_threads = 4,verbose = FALSE)
print(p12)
```

```{r tsne-plot2, fig.width=8, fig.height=4.5, warning=FALSE}
p13 <- my_tsne_plot(fit,k = 5:7, n = 4000,num_threads = 4,verbose = FALSE)
print(p13)
```

```{r tsne-plot3, fig.width=8, fig.height=4.5, warning=FALSE}
p14 <- my_tsne_plot(fit,k = 8:10, n = 4000,num_threads = 4,verbose = FALSE)
print(p14)
```
...
```{r clustering-3, fig.width=5.5, fig.height=4.5}
p9 <- my_pca_plot(fit,pcs = 1:2,k = 1:4)
print(p9)
```

```{r clustering-4, fig.width=5.5, fig.height=4.5}
p10 <- my_pca_plot(fit,pcs = 1:2,k = 5:7)
print(p10)
```

```{r clustering-5, fig.width=5.5, fig.height=4.5}
p11 <- my_pca_plot(fit,pcs = 1:2,k = 8:10)
print(p11)
```
