...
Initialized from coneNMF (iter100) and ran only 20 iterations till converged (`rel.tol = 1e-04`)
```{r}
...
Is the NMF solution contained in the cone?

```{r fig.height=15, fig.width=20}
F_proj = readRDS("output/kos_coneFactor_F.Rds")$f_proj
par(mfrow = c(5, 4))
for(i in 1:k){
  f = fitted$A[,i]
  f_proj  = F_proj[[i]]
